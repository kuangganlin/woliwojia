...
Page({
  ...
  getOrderInfo: function () {
    ...
    util.httpUtil.httpGet(util.httpUtil.url.GetOrderInfoAddPrice_Url, postData, function (res) {
  ...
  payGoodsHandle: function () {
    ...
    var that = this;
    var postData = {};
    //订单追加费用才调用order_id、add_id这两个值,普通订单支付不用调用
    postData.order_id = this.data.orderInfo.order_id;//因为是订单追加费用,所以，最好是要取得这个值
    postData.add_id = this.data.orderInfo.add_id;//因为是订单追加费用,所以，最好是要取得这个值
    ...
    } else if (postData.pay_type == 4) {//虚拟币支付
  ...
  payGoods: function (postData) {
    var that = this;
    util.httpUtil.httpPost(util.httpUtil.url.PayGoodsAddPrice_Url, postData, function (res) {
      if (util.httpUtil.isRequestSuccess(res)) {
  ...
  payGoodsResult: function (index) {
    ...
    if (this.data.pageType == 1){
      ...
      if (!util.dataUtil.isNull(prevPage)){
        wx.navigateBack({});
        if (typeof prevPage.updatePage === 'function') {
          prevPage.updatePage();
        }
      }
    }else{
