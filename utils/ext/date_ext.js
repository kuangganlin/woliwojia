...
/**
 * 字符串转Date
 * @param dateString 指定的转换格式，默认为:yyyy-MM-dd hh:mm:ss
 */
module.exports.convertDateFromString = function (dateString) {
  if (dateString) {
 ...
 * @param value 增加的毫秒数 (可以是负值)
 */
Date.prototype.differenceHours = function (value) {
  if (!value || isNaN(value))
    value = new Date();
  return parseInt(this - value) / 1000 / 60 / 60;
}
 ...
 * @param value 增加的毫秒数 (可以是负值)
 */
Date.prototype.differenceDays = function (value) {
  if (!value || isNaN(value))
    value = new Date();
  return parseInt(this - value) / 1000 / 60 / 60 / 24;
}
